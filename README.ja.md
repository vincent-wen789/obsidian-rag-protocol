...
仕様は現在 v1.8.x。リポジトリには **8 本の単一ファイル Python ユーティリティ**（約 3.6k 行：stdlib + v1.6 vec 層用のオプション `openai` + `tiktoken`）+ 2 つのリファレンス hook スクリプト。
...
- [`OBSIDIAN-RAG-PROTOCOL.md`](OBSIDIAN-RAG-PROTOCOL.md) ── 完全なプロトコル仕様（v1.8.x）
